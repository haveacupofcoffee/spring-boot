...
import ca.codingforfun.storage.StorageFileNotFoundException;
import ca.codingforfun.storage.StorageService;
...
public class FileUploadController {
    ...
    public String listUploadedFiles(Model model) throws IOException {
        model.addAttribute("files", storageService.loadAll().map(path -> MvcUriComponentsBuilder.fromMethodName(
                FileUploadController.class, "serveFile", path.getFileName()
    ...
    public ResponseEntity<Resource> serveFile(@PathVariable String filename) {
        ...
        return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "attchment;filename=\"" + file.getFilename()
            + "\"").body(file);
    }
